...
if (dashboardType === "delegation") {
  // For DELEGATION mode: Different columns for admin vs regular user
  if (userRole === 'admin') {
    // Admin: Only check Column D (Given By)
    const columnD = getCellValue(row, 3) || 'Unassigned'; // Column D (index 3)
    assignedTo = columnD;
    isUserMatch = columnD.toLowerCase() === username.toLowerCase();
    
    if (rowIndex <= 5) {
      console.log(`Row ${rowIndex + 1} (DELEGATION-ADMIN): Column D="${columnD}", username="${username}", isMatch=${isUserMatch}`);
    }
  } else {
    // Regular User: Only check Column E (Name)
    const columnE = getCellValue(row, 4) || 'Unassigned'; // Column E (index 4)
    assignedTo = columnE;
    isUserMatch = columnE.toLowerCase() === username.toLowerCase();
    
    if (rowIndex <= 5) {
      console.log(`Row ${rowIndex + 1} (DELEGATION-USER): Column E="${columnE}", username="${username}", isMatch=${isUserMatch}`);
    }
  }
  
  if (!isUserMatch) {
    if (rowIndex <= 5) console.log(`Row ${rowIndex + 1}: Skipped due to user mismatch in delegation mode`);
    return null;
  }
} else {
  // For CHECKLIST mode: Column E for all users
  assignedTo = getCellValue(row, 4) || 'Unassigned'; // Column E (index 4)
  isUserMatch = userRole === 'admin' || assignedTo.toLowerCase() === username.toLowerCase();

  if (rowIndex <= 5) {
    console.log(`Row ${rowIndex + 1} (CHECKLIST): Column E="${assignedTo}", username="${username}", userRole="${userRole}", isMatch=${isUserMatch}`);
  }

  if (!isUserMatch) {
    if (rowIndex <= 5) console.log(`Row ${rowIndex + 1}: Skipped due to user mismatch in checklist mode`);
    return null;
  }
}
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {getFilteredPopupData().map(task => (
                      <tr key={task.id} className="hover:bg-gray-50">
                        ...
                        {/* <td className="px-6 py-4 text-sm text-gray-500">{task.givenBy}</td> */}
