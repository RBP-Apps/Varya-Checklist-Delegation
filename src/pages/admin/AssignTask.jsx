...
export default function AssignTask() {
  ...
  const [formData, setFormData] = useState({
    department: "",
    givenBy: sessionStorage.getItem('userFullName') || "", // Initialize with userFullName
    doer: [],
    description: "",
    frequency: "daily",
  ...
  const fetchMasterSheetOptions = async () => {
    try {
      const masterSheetId = "1WIxX_wOaCYeOc96Jhghkv6YEtWrhpM0J7kjoDRZ-fwg";
  ...
  const getLastTaskId = async (sheetName) => {
    try {
      const url = `https://docs.google.com/spreadsheets/d/1WIxX_wOaCYeOc96Jhghkv6YEtWrhpM0J7kjoDRZ-fwg/gviz/tq?tqx=out:json&sheet=${encodeURIComponent(
        sheetName
      )}`;
  ...
  const fetchWorkingDays = async () => {
    try {
      const sheetId = "1WIxX_wOaCYeOc96Jhghkv6YEtWrhpM0J7kjoDRZ-fwg";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const tasksData = generatedTasks.map((task, index) => ({
        ...
        givenBy: formData.givenBy,                // Use the formData.givenBy instead of task.givenBy
      ...
      setFormData({
        department: "",
        givenBy: sessionStorage.getItem('userFullName') || "", // Keep the current user
        doer: [], // Reset to empty array
        description: "",
        frequency: "daily",
        enableReminders: true,
        requireAttachment: false
      ...
      setSelectedDoers([]); // Add this line - it was missing
      setGeneratedTasks([]);
