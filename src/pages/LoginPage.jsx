...
//       const SCRIPT_URL = "https://script.google.com/macros/s/AKfycby_2rGoYWDROkUQmU7pBQWR_aNjqwn1LjZ5cSYdFtRgxGU9gp9jO29cXUUNpHXeJ6Mj/exec"

//       try {
//         setIsDataLoading(true)

//         // Get the spreadsheet ID from your Apps Script
//         const SPREADSHEET_ID = "1WIxX_wOaCYeOc96Jhghkv6YEtWrhpM0J7kjoDRZ-fwg"
...
const LoginPage = () => {
  ...
  const [toast, setToast] = useState({ show: false, message: "", type: "" })
  const [showSuccessPopup, setShowSuccessPopup] = useState(false)  // <-- Added success popup state
      ...
      try {
        ...
        if (data.table && data.table.rows) {
          for (let i = 1; i < data.table.rows.length; i++) {
            ...
            // Extract data from columns C (name), D (username), E (password), F (role)
            const name = row.c[2] ? String(row.c[2].v || '').trim() : ''
            const username = row.c[2] ? String(row.c[2].v || '').trim().toLowerCase() : ''
            const password = row.c[3] ? String(row.c[3].v || '').trim() : ''
            const role = row.c[4] ? String(row.c[4].v || '').trim() : 'user'
        ...
        console.error("Error Fetching Master Data:", error)
        setToast({ show: true, message: `Network error: ${error.message}. Please try again later.`, type: "error" })
      } finally {
        setIsDataLoading(false)
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (trimmedUsername in masterData.userCredentials) {
        ...
        if (correctPassword === trimmedPassword) {
          sessionStorage.setItem('username', trimmedUsername)
          sessionStorage.setItem('userFullName', userName)
          const isAdmin = userRole === "admin"
          sessionStorage.setItem('role', isAdmin ? 'admin' : 'user')
          if (isAdmin) {
          ...
          setToast({ show: true, message: `Login successful. Welcome, ${userName}!`, type: "success" })
